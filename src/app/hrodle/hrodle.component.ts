...
@Component({
  selector: 'app-hrodle',
  templateUrl: './hrodle.component.html',
  styleUrls: ['./hrodle.component.css']
})
export class HrodleComponent implements OnInit {

  @HostListener('document:keydown', ['$event'])
  handleKeyboardEvent(event: KeyboardEvent) {
    if((event.keyCode > 64 && event.keyCode < 91 && event.keyCode != 81 && event.keyCode != 87 && event.keyCode != 88 && event.keyCode != 89) ||
      (event.key === 'Enter') ||
      (event.key === 'Backspace') ||
      (event.key === 'Č') ||
  ...
  alphabet : Letter[] = [
    ...
    {state: State.EMPTY, letter: 'Š'},
    {state: State.EMPTY, letter: 'Đ'}
  ];
  alphabet2 : Letter[] = [
  ...
  alphabet3 : Letter[] = [
    ...
    {state: State.EMPTY, letter: 'Č'},
    {state: State.EMPTY, letter: 'Ć'}
  ];
  ...
  dateZero : Date = new Date('04/10/2022');
  ...
  checkWord(word){
    ...
    if(isWord){
      ...
      document.getElementById("nepostoji").innerHTML = 'Nije na popisu riječi!';
  ...
  setColors(){
    ...
    for(let i = 0; i<this.alphabet3.length; i++){
      if(this.alphabet3[i].state === State.WHITE) keys[i+21].classList.add('toGrayKey');
      else if(this.alphabet3[i].state === State.RED) keys[i+21].classList.add('toRedKey');
      else if(this.alphabet3[i].state === State.BLUE) keys[i+21].classList.add('toBlueKey');
    }
  ...
  copyClipboard(){
    ...
    let seconds = now.getTime() - this.dateZero.getTime();
    let days = Math.round(seconds / (1000 * 3600 * 24));
    let temp = `HRodle #${days+1} - ${now.getDate()}.${now.getMonth() + 1}.${now.getFullYear()}. \n`;
